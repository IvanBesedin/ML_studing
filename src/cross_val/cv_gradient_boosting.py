# ============================================================================
# src/cv_gradient_boosting.py
# GRADIENT BOOSTING + RANDOMIZED SEARCH CV — ТОЛЬКО МЕТРИКИ
# ============================================================================
import json
import os
import time
...
def create_param_grid(cv_params: dict) -> dict:
    ...
    param_distributions = {
        'n_estimators': cv_params.get('n_estimators', [100, 150, 200, 300, 500]),
        'max_depth': cv_params.get('max_depth', [3, 5, 7, 10, 15]),
        'min_samples_split': cv_params.get('min_samples_split', [10, 20, 50, 100]),
        'min_samples_leaf': cv_params.get('min_samples_leaf', [5, 10, 20, 50]),
        'learning_rate': cv_params.get('learning_rate', [0.01, 0.05, 0.1, 0.2]),
        'subsample': cv_params.get('subsample', [0.7, 0.8, 0.9, 1.0]),
        'max_features': cv_params.get('max_features', ['sqrt', 'log2', None, 0.5, 0.7, 0.9]),
        'min_impurity_decrease': cv_params.get('min_impurity_decrease', [0.0, 0.001, 0.01, 0.1]),
    }
...
def extract_cv_results(random_search) -> dict:
    ...
    cv_results = {
        ...
        'top_10_params': results_df[['rank_test_score', 'mean_test_score', 'std_test_score', 
                                      ...
                                      'param_min_samples_leaf']].to_dict('records'),
...
def print_cv_report(cv_results: dict, test_metrics: dict, total_time: float):
    ...
    for i, row in enumerate(cv_results['top_10_params'][:3], 1):
        print(f"   #{i} Score: {row['mean_test_score']:.6f} ± {row['std_test_score']:.6f} | "
              ...
              f"lr={row['param_learning_rate']}, subsample={row['param_subsample']}, "
...
def main():
    ...
    metrics_output = "metrics/cv_gradient_boosting_metrics.json"

    # Загрузка параметров
    print(f"\n📂 Загрузка параметров из: {params_path}")
    all_params = load_params(params_path)
    cv_params = all_params.get("cv_gradient_boosting", {})
