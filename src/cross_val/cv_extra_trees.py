# ============================================================================
# EXTRA TREES + RANDOMIZED SEARCH CV — ТОЛЬКО МЕТРИКИ
# ============================================================================
import json
import os
import time
...
def create_param_grid(cv_params: dict) -> dict:
    ...
    custom_class_weights = [{0: 1, 1: w} for w in cv_params.get('class_weights', [2, 4, 8, 10, 12])]
    custom_class_weights.extend([None, 'balanced'])
    
    param_distributions = {
        'n_estimators': cv_params.get('n_estimators', [100, 200, 300, 500]),
        'max_depth': cv_params.get('max_depth', [10, 20, 30, None]),
        'min_samples_split': cv_params.get('min_samples_split', [2, 5, 10, 20]),
        'min_samples_leaf': cv_params.get('min_samples_leaf', [1, 5, 10]),
        'max_features': cv_params.get('max_features', ['sqrt', 'log2', None, 0.5, 0.7]),
        'bootstrap': cv_params.get('bootstrap', [True, False]),
...
def run_randomized_search(X_train, y_train, param_distributions: dict, cv_params: dict) -> RandomizedSearchCV:
    ...
    random_search = RandomizedSearchCV(
        ...
        n_iter=cv_params.get('n_iter', 500),
        cv=cv_strategy,
...
def extract_cv_results(random_search) -> dict:
    ...
    # Анализ структуры леса
    avg_depth = float(np.mean([tree.get_depth() for tree in best_model.estimators_]))
    avg_leaves = float(np.mean([tree.get_n_leaves() for tree in best_model.estimators_]))
...
def print_cv_report(cv_results: dict, test_metrics: dict, total_time: float):
    ...
    print(f"   • Средняя глубина: {cv_results['avg_tree_depth']:.1f}")
    print(f"   • Средняя листьев: {cv_results['avg_tree_leaves']:.0f}")
...
def main():
    ...
    metrics_output = "metrics/cv_extra_trees_metrics.json"

    # Загрузка параметров
    print(f"\n📂 Загрузка параметров из: {params_path}")
    all_params = load_params(params_path)
    cv_params = all_params.get("cv_extra_trees", {})
