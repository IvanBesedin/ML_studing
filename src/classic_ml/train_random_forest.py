...
def train_model(X_train, y_train, params: dict) -> RandomForestClassifier:
    ...
    params — любой словарь параметров для RandomForestClassifier
    """
    ...
    rf = RandomForestClassifier(**valid_params)
...
def main():
    ...
    rf_params = all_params.get("random_forest", {})
    print(f"   Параметры модели: {rf_params}")
    ...
    metrics_rf = {
        ...
        'ROC-AUC': float(roc_auc_score(y_test, y_proba_rf)),
        'PR-AUC': float(average_precision_score(y_test, y_proba_rf)),
    ...
    save_metrics(metrics_rf, metrics_output)
