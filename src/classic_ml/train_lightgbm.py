...
def train_model(X_train, y_train, params: dict) -> lgb.LGBMClassifier:
    """
    Обучение модели LightGBM.
    params — любой словарь параметров для LGBMClassifier
    """
    ...
    lgbm = lgb.LGBMClassifier(**valid_params)
...
def main():
    ...
    model_output = "models/lightgbm.pkl"
    metrics_output = "metrics/lightgbm_metrics.json"

    # Загрузка параметров
    print(f"\n📂 Загрузка параметров из: {params_path}")
    all_params = load_params(params_path)
    lgbm_params = all_params.get("lightgbm", {})
    print(f"   Параметры модели: {lgbm_params}")
    ...
    metrics_lgbm = {
        ...
        'Precision': float(precision_score(y_test, y_pred_lgbm, zero_division=0)),
        'Recall': float(recall_score(y_test, y_pred_lgbm, zero_division=0)),
        'F1-Score': float(f1_score(y_test, y_pred_lgbm, zero_division=0)),
        'ROC-AUC': float(roc_auc_score(y_test, y_proba_lgbm)),
        'PR-AUC': float(average_precision_score(y_test, y_proba_lgbm)),
    ...
    save_metrics(metrics_lgbm, metrics_output)
